...
describe("ProductPurchasePanel", () => {
  it("renders product purchase information", () => {
    ...
    expect(screen.getByText("34 disponibles")).toBeDefined();
    expect(screen.getByText(/2 opiniones/)).toBeDefined();
    expect(screen.getByText("Ships in 2 weeks")).toBeDefined();
    expect(screen.getByText("1 year warranty")).toBeDefined();
    expect(screen.getByText(/7 days return policy/i)).toBeDefined();
    expect(screen.getByText("Compra mínima: 20 unidades.")).toBeDefined();

    expect(
      screen.getByRole("button", { name: "Comprar ahora" }),
    ).toBeDefined();
